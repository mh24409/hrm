...
@section('title', localize('update_attendance'))
@section('content')
    @include('humanresource::attendance_header')
    <div class="card mb-4 fixed-tab-body">
        @include('backend.layouts.common.validation')
        @include('backend.layouts.common.message')
        <input type="hidden" id="missingAttnStore" value="{{ route('attendances.editAttendance.store') }}">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h6 class="fs-17 fw-semi-bold mb-0">{{ localize('update_attendance') }}</h6>
                </div>
                <div class="text-end">
                    <div class="actions">
        ...
        <div class="card-body">
            <form action="{{ route('attendances.editAttendance') }}" method="GET">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <tbody>
                            @foreach ($attendance as $key => $value)
                                <tr>
                                    ...
                                    <td>
                                        {{ isset($value->position->position_name) ? $value->position->position_name : localize('no_position_found') }}
                                    </td>
                                    <td>
                                        <input type="time"
                                            value="{{ isset($value->attendances[0]->time) ? \Carbon\Carbon::parse($value->attendances[0]->time)->format('H:i') : '' }}"
                                            class="form-control in_time" name="in_time[]" />
                                    </td>

                                    <td>
                                        <input type="time"
                                            value="{{ isset($value->attendances[1]->time) ? \Carbon\Carbon::parse($value->attendances[1]->time)->format('H:i') : '' }}"
                                            class="form-control out_time" name="out_time[]" />
                                    </td>
