...
@section('title', localize('check-in'))
@section('content')
    @include('humanresource::attendance_header')
    <div class="card mb-4 fixed-tab-body">
        @include('backend.layouts.common.validation')
        @include('backend.layouts.common.message')
        <input type="hidden" id="missingAttnStore" value="{{ route('attendances.checkOut.store') }}">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h6 class="fs-17 fw-semi-bold mb-0">{{ localize('check-in') }}</h6>
                </div>
                <div class="text-end">
                    <div class="actions">
                        <a href="{{ route('attendances.create') }}" class="btn btn-success btn-sm"><i
                                class="fa fa-list"></i>&nbsp;{{ localize('check-in') }}</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('attendances.checkOut.store') }}" method="GET">
                <div class="row">
                    <div class="col-md-6">
